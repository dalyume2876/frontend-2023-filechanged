class AsideEvent {
    ...
    public static getInstance(): AsideEvent {
        // instance가 null인 경우(인스턴스 생성되지 않은 경우에서만)
        // : 새 인스턴스를 생성 
    ...
    public addEventShowMenuButton(): void {
        ...
        if(menuButton) {
            menuButton?.addEventListener('click', () => {
                ...
                // menuAside에 'hidden-menu' 클래스가 있으면, 해당 클래스를 제거하고, 메뉴의 버튼 텍스트를 ◀로 변경
    ...
    public addEventMainChance(): void {
        ...
        menuItems.forEach((menuItems, index) => {
            ...
            menuItems.onclick = () => {
                ...
                // mainAside에 hidden-menu 클래스를 추가하여 메뉴를 숨김
                menuAside?.classList.add('hidden-menu')
            }
        })
    }
